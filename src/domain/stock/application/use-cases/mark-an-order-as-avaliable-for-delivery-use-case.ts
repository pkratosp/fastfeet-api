...
type MarkAnOrderAvaliableForDeliveryUseCaseResponse = Either<
  Error,
  {
...
export class MarkAnOrderAvaliableForDeliveryUseCase {
  ...
  async execute(
    data: MarkAnOrderAvaliableForDeliveryUseCaseRequest,
  ): Promise<MarkAnOrderAvaliableForDeliveryUseCaseResponse> {
    const findPackage = await this.packageRepository.findByTrackingNumber(
      data.trackingNumber,
    );
